...
class ImageDetailViewModel extends ChangeNotifier {
  ...
  ImageDetailViewModel(String photoUrl, this._photoId) {
    _initialize(photoUrl);
  }
  ...
  // Load saved colors from Firestore
  Future<void> _loadSavedColors() async {
    if (userId == null) return;
    try {
      final doc = await _firestore
          .collection('user_color_preferences')
          .doc(userId)
          .collection('photo_colors')
          .doc(_photoId)
          .get();
      ...
      if (doc.exists) {
        ...
        if (data['textColor'] != null) {
          _textColor = Color(data['textColor']);
        }
      }
    } catch (e) {
  ...
  // Save selected colors to Firestore
  Future<void> saveColors(Color backgroundColor, Color textColor) async {
    if (userId == null) return;
  ...
  // Reset colors to auto-generated
  Future<void> resetToAutoColors(String photoUrl) async {
    final autoColor = await _getDominantColor(photoUrl);
    await saveColors(autoColor, Colors.white);
  }
  ...
  Future<void> _fetchDominantColor(String imageUrl) async {
    final color = await _getDominantColor(imageUrl);
    // Only set if no saved colors exist
    if (_backgroundColor == Colors.grey.shade800) {
      _backgroundColor = color;
      notifyListeners();
    }
  }
